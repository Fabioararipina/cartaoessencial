...
import {
  ...
  Login as LoginIcon, Add as AddIcon, ArrowForward as ArrowForwardIcon
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    setUrlMessage(''); // Limpa a mensagem da URL ao tentar logar
    setLoading(true);
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh' }}>
      ...
      >
        <Box
          sx={{
            ...
            '&::before': { top: '10%', left: '10%', width: 300, height: 300 },
            '&::after': { bottom: '10%', right: '10%', width: 400, height: 400 },
          }}
        />
        ...
        <Box sx={{ position: 'relative', zIndex: 1, maxWidth: 500 }}>
          ...
          <Box sx={{ '& > div': { display: 'flex', alignItems: 'center', gap: 2, mb: 2 } }}>
            {[
              ...
              <Box key={i}>
                <Box sx={{ width: 48, height: 48, borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px' }}>
                  {item.icon}
                </Box>
      ...
      <Container component="main" maxWidth="xs" sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', flexGrow: 1, bgcolor: 'background.default', py: 4, px: { xs: 2, sm: 3, lg: 0 } }}>
        <Paper elevation={3} sx={{ p: { xs: 3, sm: 4 }, width: '100%', borderRadius: '12px' }}>
          ...
          <Box sx={{ textAlign: { xs: 'center', lg: 'left' }, mb: 4 }}>
            <Typography variant="h4" component="h2" fontWeight="bold" sx={{ mb: 1 }}>Acesse sua conta</Typography>
            <Typography variant="body1" color="text.secondary">Entre para continuar aproveitando seus benefícios</Typography>
          </Box>
          <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}>
            {urlMessage && (<Alert severity="warning" sx={{ mb: 2 }}>{urlMessage}</Alert>)}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
              <FormControlLabel control={<Checkbox value="remember" color="primary" />} label="Lembrar-me" sx={{ '& .MuiFormControlLabel-label': { fontSize: '0.875rem' } }}/>
              <Link component={RouterLink} to="/esqueci-senha" variant="body2" sx={{ ml: 'auto' }}>Esqueceu a senha?</Link>
            </Box>
            <Button type="submit" fullWidth variant="contained" sx={{ mt: 2, mb: 2, py: 1.5 }} disabled={loading} endIcon={loading ? null : <ArrowForwardIcon />}>
              {loading ? <CircularProgress size={20} color="inherit" /> : 'Entrar'}
            </Button>
            <Divider sx={{ my: 3 }}>ou</Divider>
            <Button component={RouterLink} to="/cadastro" fullWidth variant="outlined" sx={{ mt: 1, py: 1.5 }} startIcon={<AddIcon />}>
              Criar nova conta
            </Button>
          </Box>
          <Typography variant="body2" color="text.secondary" align="center" sx={{ mt: 3 }}>
            Ao continuar, você concorda com nossos{' '}
            <Link component={RouterLink} to="/termos-de-uso" color="primary">Termos de Uso</Link>{' '}
            e{' '}
            <Link component={RouterLink} to="/politica-de-privacidade" color="primary">Política de Privacidade</Link>
          </Typography>
        </Paper>
      </Container>
    </Box>
